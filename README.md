...
## ⚠️ Если появляется `hs_err_pid*.log` (краш JVM)

Если после запуска Minecraft появляется файл `hs_err_pid*.log`, это обычно не ошибка логики SBuild, а падение JVM/драйвера.
Рекомендуется:

- обновить Java 21 до свежего патча (Temurin/Zulu),
- обновить драйвер видеокарты,
- временно запускать с `-XX:TieredStopAtLevel=1` (обход редких крашей JIT на некоторых системах).

Файлы `hs_err_pid*.log` теперь игнорируются Git и не будут случайно попадать в репозиторий.
